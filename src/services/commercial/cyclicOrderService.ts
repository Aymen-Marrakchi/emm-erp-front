...
export const cyclicOrderService = {
  ...
  getDue: async (): Promise<CyclicOrder[]> => (await api.get(`${PREFIX}/due`)).data,
  ...
  ): Promise<CyclicOrder> => (await api.put(`${PREFIX}/${id}`, payload)).data,

  toggleActive: async (id: string): Promise<CyclicOrder> =>
    (await api.post(`${PREFIX}/${id}/toggle`)).data,

  fire: async (id: string): Promise<{ cyclicOrder: CyclicOrder; salesOrder: unknown }> =>
